...
exp_conversion_a = st.number_input(
    "Expected conversions for group A", min_value=0.0, max_value=1.0,
    value=0.03)
exp_conversion_b = st.number_input(
    "Expected conversions for group B", min_value=0.0, max_value=1.0,
    value=0.02)
alpha = st.slider("Confidence level", min_value=0.0, max_value=1.0, value=0.05)
power = st.slider(
    "Desired statistical power", min_value=0.0, max_value=1.0, value=0.9)
if st.button("Compute"):
    with st.spinner("Computing results..."):
        total_pop, sample_a, sample_b, pval, power = \
            doe.optimal_experiment_test_prop(
        ...
        if pval <= 0.05:
            ...
            \nThe expected p-value for this experiment is \
            {round(pval,4)}"""
            )
        else:
            st.warning(
                """The experiment may not yield significant results with \
                    the inputed parameters.
            \nTry changing the expected conversions, alpha or test power\
