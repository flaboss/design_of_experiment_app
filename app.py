...
st.text(
    """This tool is meant to help in the process of crafting a DOE
...
st.text(
    """Ex.: A marketing manager has a population of 50000 people to mail a letter
...
population = st.number_input("Total population available", min_value=1, value=50000)
exp_conversion_a = st.number_input(
    "Expected conversions for group A", min_value=0.0, max_value=1.0, value=0.03
)
exp_conversion_b = st.number_input(
    "Expected conversions for group B", min_value=0.0, max_value=1.0, value=0.02
)
alpha = st.slider("Confidence level", min_value=0.0, max_value=1.0, value=0.05)
power = st.slider("Desired statistical power", min_value=0.0, max_value=1.0, value=0.9)
if st.button("Compute"):
    with st.spinner("Computing results..."):
        ...
        try:
            if pval <= 0.05:
                st.success(
                    f"""The experiment can yield significant results with the inputed parameters.
                ...
                st.warning(
                    """The experiment may not yield significant results with the inputed parameters.
                \nTry changing the expected conversions, alpha or test power to get significant results."""
                )
        except:
